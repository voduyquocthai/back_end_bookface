...
public interface FriendshipRepo extends JpaRepository<Friendship, Long> {
    ...
    @Query(value = "SELECT * FROM mxh_bookface.friendship where (sender_user_id = ?1 and receiver_user_id = ?2)\n" +
            " or" +
    ...
    @Query(value = "SELECT * FROM mxh_bookface.friendship where receiver_user_id = ? and status = false", nativeQuery = true)
